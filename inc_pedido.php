...
try {
    // Inserir os dados do pedido na tabela pedidos
    $queryPedido = "INSERT INTO pedidos (data, id_cliente, observacao, cond_pagto, prazo_entrega) VALUES (?, ?, ?, ?, ?)";
    $stmtPedido = mysqli_prepare($con, $queryPedido);
    mysqli_stmt_bind_param($stmtPedido, "sissi", $data, $id_cliente, $observacao, $cond_pagto, $prazo_entrega);
    mysqli_stmt_execute($stmtPedido);

    // Recuperar o ID do pedido recém-inserido
    ...
    if (isset($_POST['produto_id']) && isset($_POST['quantidade'])) {
        ...
        for ($i = 0; $i < count($produto_ids); $i++) {
            ...
            mysqli_stmt_bind_param($stmtItem, "iii", $pedido_id, $produto_id, $quantidade);
    ...
    $_SESSION['msg'] = "<p style='color: green; text-align: center;'><b>Pedido cadastrado com sucesso</b></p>";
    header("Location: cad_pedido.php");
} catch (mysqli_sql_exception $exception) {
    mysqli_rollback($con);

    $_SESSION['msg'] = "<p style='color: red; text-align: center;'><b>O pedido não foi cadastrado, verifique</b></p>";
    header("Location: cad_pedido.php");
    throw $exception;
}
